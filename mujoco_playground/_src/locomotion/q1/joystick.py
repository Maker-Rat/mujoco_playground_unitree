...
from mujoco_playground._src.locomotion.q1 import base as q1_base
from mujoco_playground._src.locomotion.q1 import q1_constants as consts
...
def default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      impl="jax",
      nconmax=4 * 8192,
      njmax=32,  # Reduced for Q1 with fewer joints
  )


class Joystick(q1_base.Q1Env):
  ...
  def __init__(
    ...
    if task.startswith("rough"):
      ...
      config.njmax = 8 + 100 * 4  # Q1 has 8 joints instead of 12
    super().__init__(
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    # Q1 doesn't have floor_found sensors, detect contact via foot height
    p_f = data.site_xpos[self._feet_site_id]
    p_fz = p_f[..., -1]
    contact = p_fz < 0.05  # Contact if foot is close to ground
  ...
  def _get_obs(
    ...
    state = jp.hstack([
        ...
        noisy_gyro,  # 3
        noisy_gravity,  # 3
        noisy_joint_angles - self._default_pose,  # 8 (2 joints per leg, 4 legs)
        noisy_joint_vel,  # 8
        info["last_act"],  # 8
        info["command"],  # 3
    ])
  ...
  def _reward_pose(self, qpos: jax.Array) -> jax.Array:
    ...
    # Q1 has 2 joints per leg (thigh, calf), 4 legs = 8 joints
    weight = jp.array([1.0, 0.1] * 4)
